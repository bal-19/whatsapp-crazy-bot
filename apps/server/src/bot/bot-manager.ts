...
import { emitAnalyticsUpdate, emitBotStatus, emitNewMessage } from '../realtime/socket.js';
...
export class BotManager {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logService.write('error', 'bot_start_failed', { errorMessage: getErrorMessage(error) });
    } finally {
  ...
  private async scheduleReconnect(): Promise<void> {
    ...
    const delayMs = Math.min(30_000, 5_000 * 2 ** Math.min(this.reconnectAttempts - 1, 3));
    logService.write('info', 'bot_reconnect_scheduled', { delayMs, attempt: this.reconnectAttempts });
  ...
  private async handleMessage(message: proto.IWebMessageInfo): Promise<void> {
    ...
    if (!text) {
      await this.sendAndLog(jid, 'Gambar/media masuk. Gw belum bisa lihat isinya, tapi vibes-nya udah gw nilai duluan. 😌', null, 0);
      return;
    }
    ...
    logService.write('info', 'message_received', { contactId: jid, inputLength: text.length });
    ...
    await this.sendAndLog(jid, result.reply, result.aiModel, result.latencyMs);
  }

  private async sendAndLog(jid: string, body: string, aiModel: string | null, latencyMs: number | null): Promise<Message> {
