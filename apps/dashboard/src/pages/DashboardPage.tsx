        ...
        <div className="space-y-6 sm:space-y-7 lg:space-y-8">
            <Card className="mesh-card overflow-hidden">
                <CardContent className="grid gap-6 p-6 sm:gap-7 sm:p-7 lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-8 lg:p-8">
                    ...
                    <div className="rounded-[1.5rem] border border-emerald-100 bg-white/80 p-5 shadow-sm sm:p-6">
                        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">Quick Pulse</p>
                        <div className="mt-5 space-y-3.5">
                            <div className="flex items-center justify-between rounded-2xl bg-emerald-50 px-5 py-3.5">
                                <span className="text-sm text-slate-600">Status koneksi</span>
                                <span className="text-sm font-semibold text-slate-900">{status === 'connected' ? 'Online' : status}</span>
                            </div>
                            <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-5 py-3.5">
                                <span className="text-sm text-slate-600">Pesan hari ini</span>
                                <span className="text-sm font-semibold text-slate-900">{totalMessagesToday}</span>
                            </div>
                            <div className="flex items-center justify-between rounded-2xl bg-amber-50 px-5 py-3.5">
                                <span className="text-sm text-slate-600">Queue Gemini</span>
                                <span className="text-sm font-semibold text-slate-900">{queueSize}</span>
                            </div>
                        </div>
                    </div>
                </CardContent>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3.5">
                        {conversations.slice(0, 5).map((conversation) => (
                            <div key={conversation.contact_id} className="rounded-[1.25rem] border border-white/70 bg-white/80 p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-6 sm:gap-7 2xl:grid-cols-[minmax(0,1fr)_340px]">
                ...
                <Card className="mesh-card">
                    ...
                    <CardContent className="space-y-3.5 text-sm text-slate-600">
                        <div className="rounded-2xl bg-white/80 p-4 leading-relaxed">
