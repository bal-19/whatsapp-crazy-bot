...
import { geminiQueue, incrementDailyCounter, isQueueOverloaded } from './rate-limiter.js';
...
export async function generateBotReply(input: GenerateReplyInput): Promise<GenerateReplyResult> {
  if (isQueueOverloaded()) {
    return { reply: ERROR_MESSAGES.queue_full, latencyMs: 0, aiModel: env.GEMINI_MODEL };
  }
  ...
  try {
    const raw = await geminiQueue.add(async () => {
      ...
      return generateGeminiReply(systemPrompt, memory.getHistory(input.contactId), input.message);
    });
    ...
    logService.write('error', 'gemini_error', { contactId: input.contactId, errorMessage: message });
    return { reply: classifyGeminiError(message), latencyMs, aiModel: env.GEMINI_MODEL };
  }
}
...
function classifyGeminiError(message: string): string {
  ...
  if (/429|rate|resource exhausted/i.test(message)) return ERROR_MESSAGES.rate_limit;
